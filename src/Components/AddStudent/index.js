...
const AddStudent = () => { 
    ...
    const handleSubmit = (e) => {
        ...
        const payload = {"name":name.trim(), "batch":batch.toUpperCase(), "gender":gender, "hasMentor":"false", 
                        assigned_Mentor:{},previous_Mentor:{} }; 
        const response = addStudentDB(payload);
        response
        .then((res)=> {if(res.data.added.acknowledged || res.status === 200 || res.status === 201 )
            navigateP(`/students`);
        })
